...
require 'random-port'
RSpec.describe 'Excon Integration' do
  ...
  around do |example|
    begin
      ...
      EtAzureInsights::Adapters::Excon.uninstall
    end
  end
  ...
  context '2 service chain with 1st failing' do
    # Here, we will configure 2 rack applications
    # Our test suite calls the first rack app using net/http with the tracking disabled (as we are not testing this part)
    # The first rack app will call the second using typhoeus
    #
    ...
    it 'inform insights of the start of the chain with a failed status' do
      rack_servers.get_request(:app1, '/anything')
  ...
  context '2 service chain with 2nd failing' do
    # Here, we will configure 2 rack applications
    # Our test suite calls the first rack app using net/http with the tracking disabled (as we are not testing this part)
    # The first rack app will call the second using typhoeus
    #
    include_context 'rack servers' do
      rack_servers.register(:app1) do |env|
        url = rack_servers.base_url_for(:app2)
        Excon.get(url)
        [500, {}, ['Internal Server Error Caused By App2']]
      end
      rack_servers.register(:app2) do |env|
        [500, {}, ['Internal Server Error']]
      end
    ...
    it 'informs insights of the second request linked to the dependency with the failed status' do
      rack_servers.get_request(:app1, '/anything')
